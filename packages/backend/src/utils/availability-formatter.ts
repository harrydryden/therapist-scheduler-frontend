...
/**
 * Format availability as time ranges grouped by week for email body
 *
 * Instead of listing individual hourly slots, this shows availability as ranges:
 * - Tuesday 10th February, 12pm-4pm
 * - Wednesday 11th February, 1pm-4pm
 */
export function formatAvailabilityForEmail(
  availability: TherapistAvailability | Record<string, unknown> | null,
  maxDays: number = 6,
  referenceDate: Date = new Date()
): string {
  ...
  // Use referenceDate for consistent time calculations with formatAvailabilityForUser
  const now = referenceDate;
  const minStartTime = new Date(now.getTime() + 2 * 60 * 60 * 1000); // 2 hour buffer
  const weeksAhead = 3;
  const endDate = new Date();
  endDate.setDate(endDate.getDate() + weeksAhead * 7);

  // Generate concrete availability windows (day + time range)
  const windows: Array<{
    date: Date;
    dayName: string;
    start: string;
  ...
  for (const slot of normalizedAvailability.slots) {
    const dayIndex = DAY_INDEX[slot.day.toLowerCase()];
    ...
    // Generate windows for each week
    while (currentDay < endDate) {
      const exceptionStatus = isExceptionDay(currentDay, normalizedAvailability.exceptions);

      if (exceptionStatus !== false) {
        // Check if this day is after our minimum start time
        const dayStart = new Date(currentDay);
        const [startHour, startMinute] = slot.start.split(':').map(Number);
        dayStart.setHours(startHour, startMinute, 0, 0);

        if (dayStart > minStartTime) {
          windows.push({
            date: new Date(currentDay),
            dayName: slot.day,
            start: slot.start,
            end: slot.end,
            display: `${formatDateLong(currentDay)}, ${formatTimeRange(slot.start, slot.end)}`,
          });
        }
      }
  ...
  const thisWeek = windows.filter(w => w.date >= thisWeekStart && w.date < nextWeekStart);
  const nextWeek = windows.filter(w => w.date >= nextWeekStart && w.date < weekAfterStart);
  const later = windows.filter(w => w.date >= weekAfterStart);

  const parts: string[] = [];

  if (thisWeek.length > 0) {
    parts.push('**This week:**');
    thisWeek.slice(0, maxDays).forEach(w => parts.push(`- ${w.display}`));
  }

  if (nextWeek.length > 0) {
    parts.push('**Next week:**');
    nextWeek.slice(0, maxDays).forEach(w => parts.push(`- ${w.display}`));
  }

  if (later.length > 0) {
    parts.push('**Later:**');
    later.slice(0, maxDays).forEach(w => parts.push(`- ${w.display}`));
  }
...
/**
 * Format a time range as "12pm-4pm"
 */
function formatTimeRange(start: string, end: string): string {
  ...
  return `${formatTimeFromStr(start)}-${formatTimeFromStr(end)}`;
}

/**
 * Check if any slots are potentially conflicting with pending appointments
 *
 * @param slots - Generated slots to check
 * @param pendingSlots - Slots that are tentatively held by other users
 * @returns List of slots that may become unavailable
 */
export function findPotentialConflicts(
  ...
  for (const slot of slots) {
    for (const pending of pendingSlots) {
      // Check if slots overlap (within session duration)
      const diff = Math.abs(slot.getTime() - pending.getTime());
      if (diff < SLOT_DURATION_MINUTES * 60 * 1000) {
        conflicts.push(slot);
        break;
      }
    }
...
/**
 * Validate that a user-selected slot matches available options
 *
 * @param selectedText - User's text selection (e.g., "Monday at 10am")
 * @param availability - Therapist's availability
 * @returns The matching slot datetime if valid, null otherwise
 */
export function validateSlotSelection(
  ...
  const formatted = formatAvailabilityForUser(availability);
  const allSlots = [
    ...formatted.thisWeek,
    ...formatted.nextWeek,
    ...formatted.later,
  ...
  const normalizedInput = selectedText.toLowerCase().trim();

  for (const slot of allSlots) {
    // Check various matching formats
    const matchFormats = [
      slot.display.toLowerCase(),
      slot.shortDisplay.toLowerCase(),
      // Also try partial matches
      formatDateShort(slot.datetime).toLowerCase(),
    ];

    for (const format of matchFormats) {
      if (normalizedInput.includes(format) || format.includes(normalizedInput)) {
