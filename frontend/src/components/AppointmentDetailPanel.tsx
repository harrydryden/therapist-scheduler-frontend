  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="h-full flex flex-col">
        ...
        {/* Conversation - latest message from each sender */}
        <div className="flex-1 overflow-y-auto p-4 max-h-[450px]">
          <h3 className="font-medium text-slate-700 mb-3">Latest Messages</h3>
          {(() => {
            const conv = appointmentDetail.conversation;
            if (!conv || conv.latestMessages.length === 0) {
            ...
            const order = ['client', 'therapist', 'agent', 'admin'];
            const sorted = [...conv.latestMessages].sort(
              (a, b) => order.indexOf(a.senderType) - order.indexOf(b.senderType)
            );
              ...
              <div className="space-y-3">
                {sorted.map((msg) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-800 whitespace-pre-wrap line-clamp-6">{sanitizeText(msg.content)}</p>
                  </div>
                ))}
                <p className="text-xs text-slate-400 text-center">
                  {conv.totalMessageCount} total messages in conversation
                </p>
              </div>
            );
          })()}
